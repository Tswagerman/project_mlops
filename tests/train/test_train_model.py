...
def test_train_with_zero_epochs():
    config = {"batch_size": 16, "lr": 0.001, "n_epochs": 0, "scheduler_step": 100, "accumulation_steps": 2, "num_workers": 4}
...
def test_train_with_negative_workers():
    config = {"batch_size": 16, "lr": 0.001, "n_epochs": 0, "scheduler_step": 100, "accumulation_steps": 2, "num_workers": -1}
...
def test_train_with_different_learning_rates():
    learning_rates = [0.001, 0.01, 0.1]
    for lr in learning_rates:
        config = {"batch_size": 16, "lr": lr, "n_epochs": 1, "scheduler_step": 100, "accumulation_steps": 2, "num_workers": 4}
        try:
            train(config)
        except Exception as e:
            pytest.fail("Config file thrown an exception it shouldn't!")
