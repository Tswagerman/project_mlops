...
class ToxicCommentsDataset(Dataset):
    def __init__(self, data: pd.DataFrame, tokenizer: BertTokenizer, max_token_len: int = 512, label_cols: list = None):
...
class ToxicCommentDataModule(pl.LightningDataModule):
    ...
    def setup(self, stage=None):
        self.train_dataset = ToxicCommentsDataset(self.train_df, self.tokenizer, self.max_token_len, self.labels)

        self.test_dataset = ToxicCommentsDataset(self.test_df, self.tokenizer, self.max_token_len, self.labels)

    def train_dataloader(self):
        return DataLoader(
            self.train_dataset, batch_size=self.batch_size, persistent_workers=True, shuffle=True, num_workers=4
        )
...
class ToxicCommentTagger(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        self.training_step_outputs.update({"loss": loss, "predictions": outputs, "labels": labels})
...
if __name__ == "__main__":
    ...
    train_dataset = ToxicCommentsDataset(train_df, tokenizer, max_token_len=512, label_cols=LABEL_COLUMNS)
    ...
    data_module = ToxicCommentDataModule(
        train_df, valid_df, tokenizer, batch_size=BATCH_SIZE, max_token_len=512, labels=LABEL_COLUMNS
    )
    ...
    trainer = pl.Trainer(logger=logger, callbacks=[early_stopping_callback, checkpoint_callback], max_epochs=N_EPOCHS)

    trainer.fit(model, data_module)

    # Save the best model checkpoint
    best_checkpoint_path = checkpoint_callback.best_model_path
    saved_model_path = "saved_model.pth"  # Adjust the path as needed

    # Copy the best checkpoint to the desired location
    os.rename(best_checkpoint_path, saved_model_path)
